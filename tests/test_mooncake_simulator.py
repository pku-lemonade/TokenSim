...
class MooncakeConfigTransferTest(unittest.TestCase):
    def test_config_files_parse_and_invalid_protocol_fails(self):
        for path in (
            ...
            "./data/kv_transfer/mooncake_store_h100_ssd_hbf1.json",
            "./data/kv_transfer/mooncake_store_h100_ssd_hbf1-48gib.json",
            "./data/kv_transfer/mooncake_store_h100_ssd_hbf2.json",
            "./data/kv_transfer/mooncake_store_h100_hbf1.json",
            "./data/kv_transfer/mooncake_store_h100_hbf1-48gib.json",
            "./data/kv_transfer/mooncake_store_h100_hbf2.json",
            "./data/kv_transfer/mooncake_store_b200_ssd_hbf1.json",
            "./data/kv_transfer/mooncake_store_b200_ssd_hbf1-48gib.json",
            "./data/kv_transfer/mooncake_store_b200_ssd_hbf2.json",
            "./data/kv_transfer/mooncake_store_b200_hbf1.json",
            "./data/kv_transfer/mooncake_store_b200_hbf1-48gib.json",
            "./data/kv_transfer/mooncake_store_b200_hbf2.json",
        ...
        with self.assertRaises(ConfigurationError):
            KVTransferConfig(
                ...
                kv_connector_extra_config={
                    "offload_tier": "hbf",
                    "hbf_capacity_blocks": -1,
                },
            )

    def test_hbf_paired_config_presets(self):
        pairs = (
            ("H100", "HBF1", 24 * 1024, 1200.0),
            ("H100", "HBF1-48GiB", 24 * 1024, 1200.0),
            ("H100", "HBF2", 12 * 1024, 600.0),
            ("B200", "HBF1", 48 * 1024, 2400.0),
            ("B200", "HBF1-48GiB", 48 * 1024, 2400.0),
            ("B200", "HBF2", 24 * 1024, 1200.0),
        )
        for hardware, preset, capacity_gib, bandwidth_gbps in pairs:
            with self.subTest(hardware=hardware, preset=preset):
                stem = hardware.lower()
                hbf = parse_mooncake_config(
                    KVTransferConfig.from_file(
                        f"./data/kv_transfer/mooncake_store_{stem}_{preset.lower()}.json"
                    ).kv_connector_extra_config
                )
                ssd = parse_mooncake_config(
                    KVTransferConfig.from_file(
                        f"./data/kv_transfer/mooncake_store_{stem}_ssd_{preset.lower()}.json"
                    ).kv_connector_extra_config
                )

                self.assertEqual(hbf.target_hardware, hardware)
                self.assertEqual(ssd.target_hardware, hardware)
                self.assertEqual(hbf.accelerator_memory_preset, preset)
                self.assertEqual(ssd.accelerator_memory_preset, preset)
                self.assertEqual(hbf.hbf_capacity_gb, capacity_gib)
                self.assertEqual(ssd.ssd_capacity_gb, capacity_gib)
                self.assertIsNone(hbf.hbf_capacity_blocks)
                self.assertEqual(hbf.hbf_seq_read_bw_gbps, bandwidth_gbps)
                self.assertEqual(hbf.hbf_seq_write_bw_gbps, bandwidth_gbps)
                self.assertAlmostEqual(
                    hbf.hbf_random_4k_read_bw_gbps,
                    0.8 * bandwidth_gbps,
                )
                self.assertEqual(hbf.hbf_read_latency_us, 12.0)
                self.assertEqual(hbf.hbf_write_latency_us, 120.0)

    def test_hbf_default_random_bandwidth_and_size_curve(self):
        config = parse_mooncake_config(
            {
                "offload_tier": "hbf",
                "hbf_capacity_blocks": 16,
                "hbf_seq_read_bw_gbps": 100.0,
                "hbf_seq_write_bw_gbps": 50.0,
            }
        )
        tier = OffloadTier(config, block_bytes=4096)

        self.assertEqual(config.hbf_random_4k_read_bw_gbps, 80.0)
        self.assertEqual(config.hbf_random_4k_write_bw_gbps, 40.0)
        self.assertAlmostEqual(tier._hbf_bandwidth(4096, is_write=False), 80.0)
        self.assertAlmostEqual(tier._hbf_bandwidth(10 * (1 << 20), is_write=False), 100.0)
        mid_bw = tier._hbf_bandwidth(1 << 20, is_write=False)
        self.assertGreater(mid_bw, 80.0)
        self.assertLess(mid_bw, 100.0)

    def test_hbf_latency_sweep_is_monotonic(self):
        latencies = []
        key = PoolKey(KeyMetadata("m"), "a")
        for latency_us in (8, 10, 20):
            store = MooncakeStore(
                parse_mooncake_config(
                    {
                        ...
                        "enable_offload": True,
                        "offload_tier": "hbf",
                        "hbf_capacity_blocks": 4,
                        "hbf_seq_read_bw_gbps": 1024.0,
                        "hbf_seq_write_bw_gbps": 1024.0,
                        "hbf_read_latency_us": latency_us,
                        "hbf_write_latency_us": latency_us,
                    }
                ),
                block_bytes=4096,
            )
            ...
            latencies.append(store.get(hit.keys, hit.tier))
...
class MooncakeStoreModelTest(unittest.TestCase):
    ...
    def test_hbf_store_matches_ssd_admission_schedule(self):
        key0 = PoolKey(KeyMetadata("m"), "a")
        key1 = PoolKey(KeyMetadata("m"), "b")
        stores = []
        for extra in (
            ...
            store = MooncakeStore(parse_mooncake_config(extra), block_bytes=128)
            store.put([key0])
            store.put([key1])
            stores.append(store)

        ssd_store, hbf_store = stores
        ssd_hit = ssd_store.lookup([key0])
        hbf_hit = hbf_store.lookup([key0])

        self.assertEqual(ssd_hit.hit_blocks, hbf_hit.hit_blocks)
        self.assertEqual(ssd_store.stats.admission_count, hbf_store.stats.admission_count)
        self.assertEqual(ssd_store.stats.eviction_count, hbf_store.stats.eviction_count)
        self.assertEqual(ssd_store.stats.store_hit_count, hbf_store.stats.store_hit_count)
        self.assertEqual(ssd_hit.tier, "ssd")
        self.assertEqual(hbf_hit.tier, "hbf")
        self.assertGreater(hbf_store.get(hbf_hit.keys, hbf_hit.tier), 0)
        self.assertGreater(hbf_store.stats.hbf_read_blocks, 0)
        self.assertGreater(hbf_store.stats.hbf_write_blocks, 0)
        hbf_profile = hbf_store.stats.as_dict()["mooncake_offload_profiles"][0]
        self.assertEqual(hbf_profile["tier"], "hbf")
        self.assertEqual(hbf_profile["hbf_read_latency_us"], 12.0)

    def test_hbf_write_blocks_and_read_has_no_persistence_wait(self):
        config = parse_mooncake_config(
            {
                "memory_capacity_blocks": 0,
                "enable_offload": True,
                "offload_tier": "hbf",
                "hbf_capacity_blocks": 4,
                "hbf_seq_read_bw_gbps": 1200.0,
                "hbf_seq_write_bw_gbps": 1200.0,
                "hbf_read_latency_us": 12.0,
                "hbf_write_latency_us": 120.0,
            }
        )
        block_bytes = 10 * (1 << 20)
        store = MooncakeStore(config, block_bytes=block_bytes)
        key = PoolKey(KeyMetadata("m"), "blocking")
        write_latency = 120e-6 + block_bytes / (1 << 30) / 1200.0
        read_latency = 12e-6 + block_bytes / (1 << 30) / 1200.0

        timing = store.put_with_timing([key], now=10.0)
        obj = store.objects[key.to_string()]

        self.assertAlmostEqual(timing.accounting_latency, write_latency)
        self.assertAlmostEqual(timing.blocking_latency, write_latency)
        self.assertTrue(obj.persisted)

        hit = store.lookup([key], now=10.0)
        self.assertEqual(hit.hit_blocks, 1)
        self.assertAlmostEqual(
            store.get(hit.keys, hit.tier, now=10.0),
            read_latency,
        )

    def test_hbf_media_timing_and_ssd_blocking_write(self):
        block_bytes = 10 * (1 << 20)
        hbf = OffloadTier(
            parse_mooncake_config(
                {
        ...
        self.assertAlmostEqual(
            hbf.read("key").latency,
            12e-6 + block_bytes / (1 << 30) / 1200.0,
        )
        self.assertAlmostEqual(
            hbf.write("key", 1).latency,
            120e-6 + block_bytes / (1 << 30) / 600.0,
        )
    ...
    def test_offload_profiles_export_corrected_provenance(self):
        hbf_store = MooncakeStore(
            parse_mooncake_config(
                {
                    "target_hardware": "H100",
                    "accelerator_memory_preset": "HBF1",
                    "enable_offload": True,
                    "offload_tier": "hbf",
                    "hbf_capacity_gb": 24 * 1024,
                    "hbf_seq_read_bw_gbps": 1200.0,
                    "hbf_seq_write_bw_gbps": 1200.0,
                }
            ),
        ...
        hbf_profile = hbf_store.stats.as_dict()["mooncake_offload_profiles"][0]
        self.assertEqual(hbf_profile["target_hardware"], "H100")
        self.assertEqual(hbf_profile["accelerator_memory_preset"], "HBF1")
        self.assertEqual(hbf_profile["shared_offload_capacity_gib"], 24 * 1024)
        self.assertEqual(hbf_profile["hbf_read_latency_us"], 12.0)
        self.assertEqual(hbf_profile["hbf_write_latency_us"], 120.0)
        self.assertTrue(hbf_profile["write_blocking"])
        self.assertEqual(hbf_profile["write_persistence"], "blocking")
            ...
            parse_mooncake_config(
                {
                    ...
                    "enable_offload": True,
                    "offload_tier": "ssd",
                    "ssd_capacity_gb": 24 * 1024,
                }
            ),
            block_bytes=4096,
        )
        ssd_profile = ssd_store.stats.as_dict()["mooncake_offload_profiles"][0]
        self.assertEqual(ssd_profile["accelerator_memory_preset"], "HBF1")
        self.assertEqual(ssd_profile["shared_offload_capacity_gib"], 24 * 1024)
...
class MooncakeConnectorTest(unittest.TestCase):
    ...
    def test_store_connector_hbf_hits_use_same_schedule(self):
        config = KVTransferConfig(
            kv_connector="MooncakeStoreConnector",
            kv_connector_extra_config={
                "memory_capacity_blocks": 1,
        ...
        self.assertEqual(meta1.loads[0].tier, "hbf")
        connector.bind_connector_metadata(meta1)
        self.assertGreater(connector.start_load_kv(), 0)
        self.assertGreater(connector.mooncake_stats.disk_tier_hit_count, 0)
        self.assertGreater(connector.mooncake_stats.hbf_tier_hit_count, 0)
        self.assertGreater(connector.mooncake_stats.hbf_read_blocks, 0)
        profiles = connector.mooncake_stats.as_dict()["mooncake_offload_profiles"]
        self.assertEqual(profiles[0]["tier"], "hbf")
        self.assertEqual(profiles[0]["hbf_random_4k_read_bw_gbps"], 102.4)

    def test_hbf_and_ssd_saves_are_blocking(self):
        def save_latency(offload_tier: str) -> tuple[float, MooncakeStoreConnector]:
            capacity_key = f"{offload_tier}_capacity_blocks"
            ...
            return connector.wait_for_save(), connector

        hbf_latency, hbf_connector = save_latency("hbf")
        reset_mooncake_services()
        ssd_latency, ssd_connector = save_latency("ssd")

        self.assertGreater(hbf_latency, 0.0)
        self.assertAlmostEqual(hbf_connector.stats.save_wait_time, hbf_latency)
        self.assertAlmostEqual(
            hbf_connector.mooncake_stats.save_wait_time,
            hbf_latency,
        )
        self.assertAlmostEqual(hbf_connector.stats.transfer_latency, hbf_latency)
        self.assertGreater(ssd_latency, 0.0)
        self.assertAlmostEqual(ssd_connector.stats.save_wait_time, ssd_latency)
        self.assertAlmostEqual(ssd_connector.mooncake_stats.save_wait_time, ssd_latency)
...
class MooncakeEndToEndTest(unittest.TestCase):
    ...
    def test_engine_exports_mooncake_store_metrics(self):
        ...
        engine.effective_hardware_profile = {
            "base_hardware": "H100",
            "accelerator_memory_preset": "HBF1",
            "memory_type": "GDDR7",
            "capacity_gib_per_card": 32.0,
            "bandwidth_tb_s_per_card": 1.792,
            "worker_count": 8,
            "shared_offload_capacity_gib": 24 * 1024,
            "offload_tier": "hbf",
        }
        requests = [
        ...
        self.assertEqual(
            result["effective_hardware_profile"],
            engine.effective_hardware_profile,
        )
